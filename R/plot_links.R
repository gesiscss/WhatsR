...
#' @param exclude.long If TRUE, links that are longer than length will be excluded. Default is TRUE.
#' @param length Gives the number of maximum characters for exclude.long. Default is 50.
#' @param min.occur The minimum number of occurrences a link has to have to be included in the visualization. Default is 1.
...
plot_links <- function(data,
  ...
  # length must be >= 1
  if (length < 1) stop("Please provide a length of >= 1.")
  # return.data must be bool
  if (!is.logical(return.data)) stop("return.data has to be either TRUE or FALSE.")
  # LinkVec must be in data
  if (!("all" %in% LinkVec) & any(!LinkVec %in% data$URL)) stop("LinkVec has to either be \"all\" or a vector of emojis to include.")
