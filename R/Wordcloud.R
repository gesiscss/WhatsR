...
# TODO: In if (unique(data$Sender) >= 8) { : the condition has length > 1 and only the first element will be used
WAwordcloud <- function(data,
                        names = "all",
  ...
  if (comparison == TRUE) {
    ...
    if (length(unique(data$Sender)) >= 8) {
      ...
      warning("Comparison wordclouds only work for up to 8 senders")
      stop()
