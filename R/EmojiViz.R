...
# TODO: Replace Emoji names with actual Emoji in plots
EmojiViz <- function(data,
                     names = "all",
  ...
  day <- hour <- `Number of Emoji` <- ave <- total <- Var1 <- Freq <- n <- NULL
  ...
  if (plot == "bar") {
    ...
    # Visualizig the distribution of Emoji
  ...
  if ( plot == "splitbar") {
    ...
    out <-   ggplot(SumFrame, aes(x = NewSender, y = n,fill = NewEmoji)) +
      geom_bar(stat = "identity", position = position_dodge()) +
      labs(title = "Emoji sent per Person and Domain",
           subtitle = paste(starttime, " - ", endtime),
           x = "Sender",
           y = "Frequency") +
      theme(legend.title = element_text("Emoji"))
    ...
    if (length(unique(SumFrame$NewEmoji)) <= 20) {
