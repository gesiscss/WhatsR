...
#' @param aggregate.sessions If TRUE, concurrent messages of the same author are aggregated into one session. Default is TRUE.
...
plot_replytimes <- function(data,
  ...
  if (!is.logical(aggregate.sessions)) stop("aggregate.sessions has to be either TRUE or FALSE.")
  # plot must be one of the the preset options
  if (any(!plot %in% c("box", "dist", "heatmap"))) stop("The plot type has to be box, dist or heatmap.")
