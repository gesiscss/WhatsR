...
# TODO: rotate smilies in description by 90 degrees to increase readibility + Smilies by Persons as legend -> for plot = bar
# TODO: For plot = "splitbar", we need to manually add a legend
SmilieViz <- function(data,
                      names = "all",
  ...
  day <- hour <- n <- `Number of Smilies` <- ave <- total <- Var1 <- Freq <- n <- NULL
  ...
  if (plot == "bar") {
    ...
    out <- ggplot(df[df$Freq >= min.occur,],aes(x = Var1,y = Freq, fill = Var1)) +
  ...
  if ( plot == "splitbar") {
    ...
    out <-   ggplot(SumFrame, aes(x = NewSender, y = n,fill = NewSmilies)) +
    ...
    if (length(unique(SumFrame$NewSmilies)) <= 20) {
