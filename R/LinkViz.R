...
# TODO: Heatmap legend is not plotting the top 10 Links, just the first 10 from the vector
LinkViz <- function(data,
                    names = "all",
...
# shorten URLs to domain
  ...
  if (plot == "bar") {
    ...
    if (exclude.long == TRUE) {
      ...
      df <- df[sapply(as.character(df$Var1),nchar) <= length,]
  ...
  if ( plot == "splitbar") {
    ...
    out <-   ggplot(SumFrame, aes(x = NewSender, y = n,fill = NewUrls)) +
    ...
    if (length(unique(SumFrame$NewUrls)) <= 20) {
